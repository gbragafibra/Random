...
class CrossEntropyCost:
	...
	@staticmethod #No instance of the class
	def fn(a,y): #Cross entropy func
		return np.sum(np.nan_to_num(-y*np.log(a)-(1-y)*np.log(1-a)))
		# np.nan_to_num to deal with problems of logs close to 0

	@staticmethod 
	def delta(z, a, y): #derivative of cross entropy
		return (a-y)
...
class Network2:
	...
	#Initialize weights with normalization and bias with STD 1
	def default_weight_initializer(self):
		self.biases = [np.random.randn(y,1) for y in self.sizes[1:]]
		self.weights = [np.random.randn(y,x)/np.sqrt(x)
		for x,y in zip(self.sizes[:-1], self.sizes[1:])]

	#Initialization with no normalization
	...
	def SGD(self, training_data, epochs, mini_batch_size,
		eta, lmbda = 0.0, evaluation_data=None,
